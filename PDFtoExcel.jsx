...
import './index.css';
import { FileSpreadsheet, HardDrive, Upload, Download } from 'lucide-react';
...
function PDFThumbnail({ file }) {
    ...
    return thumbnail ? (
        ...
        <div className="w-full h-64 bg-gray-50 flex items-center justify-center">
            <FileSpreadsheet size={48} className="text-green-300" />
        </div>
    );
}
    ...
    return (
        <div className="flex flex-col gap-4">
            <label
                ...
                className={`border-2 border-dashed rounded-2xl p-12 flex flex-col items-center justify-center gap-4 transition-colors cursor-pointer ${dragging ? 'border-green-500 bg-green-50' : 'border-green-300 bg-white hover:bg-green-50/40'}`}
            >
                <input
                    type="file"
                    accept=".pdf"
                ...
                <div className="flex gap-3 mt-1">
                    <div className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold px-5 py-2 rounded-lg transition">
...
export default function PDFtoExcel({ onNavigate, user, setUser }) {
    ...
    const handlePDFtoExcel = () => {
        ...
        setTimeout(() => {
            const newResult = {
                id: Date.now(),
                name: files[0]?.name ? `${files[0].name.replace('.pdf', '')}.xlsx` : `Converted-Excel-${convertedHistory.length + 1}.xlsx`,
        ...
        <div className="max-w-3xl mx-auto flex flex-col gap-6">
            ...
            <div>
                <button
                    ...
                    className="text-sm hover:underline text-green-800 mb-6 flex gap-1 cursor-pointer font-medium"
                >
                    ← Back to Dashboard
                </button>
                <div className="flex flex-col gap-1 justify-center text-center" >
                    <h1 className="text-3xl font-bold text-gray-900">Convert PDF to Excel</h1>
                    <p className="text-sm text-gray-500 mt-1">Extract tabular data from PDF files to Excel spreadsheets cleanly.</p>
                </div>
            </div>
            ...
            {files.length > 0 && (
                <div className="flex justify-center">
                    <button
                        ...
                        className="bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold px-6 py-3 rounded-xl transition shadow-sm"
                    >
                        {isConverting ? 'Converting...' : isUploading ? 'Waiting for upload...' : 'Convert PDF to Excel'}
                    </button>
                </div>
            )}
                ...
                <div className="flex flex-col gap-3">
                    ...
                    {convertedHistory.map(result => (
                        <div key={result.id} className="bg-white border border-gray-200 rounded-xl px-5 py-4 flex items-center justify-between shadow-sm">
                            ...
                            <button className="bg-green-500 hover:bg-green-700 text-white text-sm font-semibold px-3 py-2 rounded-lg transition flex items-center gap-1">
                                <Download size={14} /> Download
                            </button>
                        </div>
                    ))}
                    <div className="bg-green-50 border border-green-200 rounded-xl px-5 py-4 text-green-700 font-medium text-sm shadow-sm">
                        ✓ Converted Excel spreadsheet created successfully!
                    </div>
                </div>
            )}
        </div>
